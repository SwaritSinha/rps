...
# main window
root = Tk()
root.title("Rock Paper Scissor Shoot")
...
# insert picture
user_label = Label(root, image=paper_img, bg="aqua")
comp_label = Label(root, image=paper_img_comp, bg="aqua")
...
# indicators
user_indicator = Label(root, font=("Arial", 15), text="USER", bg="aqua", fg="blue")
...
def updateCoumptuerscore():
    score = int(coumptuerscore['text'])
    score += 1 #score = score + 1
    coumptuerScore["text"] = str(score)


# check winner    
def checkWin(player,coumputer):
    if(player == coumputer):
        updateMessage("It's a tie!")
    elif(player == "rock"):
        if(coumputer == "scissor"):
            updateMessage("You won")
            updatePlayerScore()
        else:
            updateMessage("You lose")
            updatecoumptuerScore()
